...
def save_original_photo_to_drive(input_image_path, folder_id, photo_title):
    ...
    print(f"Uploaded original image to Google Drive with File ID: {file.get('id')}")
...
def generate_image(input_image_path, output_folder, n, photo_title):
    ...
    human_folder_id = "1ZjumcbLeUY7KK6nDf2y2CvzJo6CtEprh"  # Replace with the ID of the "human" folder
    original_photo_file_id = save_original_photo_to_drive(input_image_path, human_folder_id, photo_title)
    ...
    for i, image_url in enumerate(image_urls):
        ...
        # Upload the image to Google Drive
        folder_id = "15hNp5EiJbOTtRATX_7ugbdKVb-1guQdd"  # Replace with the ID of the folder where you want to save generated images
        file_metadata = {
        ...
        new_df.loc[new_df['file_url'] == input_image_path, 'original_photo_file_id'] = original_photo_file_id
        new_df.loc[new_df['file_url'] == input_image_path, 'generated_image_url'] = image_url
        new_df.loc[new_df['file_url'] == input_image_path, 'generated_image_file_id'] = file.get('id')
...
def main(output_folder, num_images):
    ...
    for _, row in selected_images.iterrows():
        ...
        generate_image(input_image_url, output_folder, 1, photo_title)
